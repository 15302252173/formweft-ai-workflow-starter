# 推广链接命名 / Review campaign tags

FORMWEFT practice card · 07/20 · 2026-09-14
...
Check campaign tags for consistent attribution names before sharing links. 发布前检查追踪参数，不把参数存在视为已有流量。

Input fields / 输入字段：`row_id, destination, utm_source, utm_medium, utm_campaign, approved_dictionary`

## Sample inputs / 样例输入

1. A: source=partner_a; medium=directory; campaign=workflow_practice.
2. B: source=Partner A; medium=Directory; campaign=workflow_practice.
3. C: source missing; medium=directory; campaign=workflow_practice. Dictionary: partner_a, directory, workflow_practice.
...
1. A: Matches dictionary.
2. B: Propose partner_a and directory, preserving original values.
3. C: Needs source identity; do not guess a partner.

**Key check / 关键检查：** The output is a naming review only. It cannot show visits or conversions. 输出行号、原值、建议值和待确认项。

## Change one input / 变式练习

Give C source=partner_b but keep dictionary unchanged: request approval to extend the dictionary instead of silently rewriting it to partner_a.
