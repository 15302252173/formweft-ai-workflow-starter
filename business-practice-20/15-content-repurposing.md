# 内容改写边界 / Repurpose an approved article

FORMWEFT practice card · 15/20 · 2026-09-14
...
Turn an approved source into short content while preserving qualifiers. 将文章改成短内容，保留限定条件。

Input fields / 输入字段：`source_id, approved_text, target_format, word_limit, required_qualifiers`

## Sample inputs / 样例输入

1. S1: 'The offline 1.5.0 package runs locally without external requests. Optional hosted AI workflows are separate.'
2. A: Requested 40-word summary.
3. B: Draft says 'The entire website never connects to external services.' C: Draft omits the version number.
...
1. A: Keep offline package, version 1.5.0 and separation from hosted AI.
2. B: Reject overgeneralization.
3. C: Restore scope/version before publishing.

**Key check / 关键检查：** Do not turn version-specific evidence into a permanent site-wide guarantee. 记录保留了哪些限定语。

## Change one input / 变式练习

Change target to a 15-word heading: if all important limits cannot fit, put the limits in a mandatory adjacent caption.
