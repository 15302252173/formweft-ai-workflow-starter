# 退款对账 / Reconcile refund events

FORMWEFT practice card · 05/20 · 2026-09-14
...
Link refund events to orders and separate pending requests from completed refunds. 区分退款申请与已完成退款。

Input fields / 输入字段：`order_id, payment_amount, currency, refund_id, refund_amount, refund_status`

## Sample inputs / 样例输入

1. Order O1 paid USD 100. Refund R1 completed USD 20; R1 appears twice in the event export.
2. Order O2 paid USD 80. Refund R2 requested USD 30, not completed.
3. Refund R3 completed USD 10 with no order_id.
...
1. O1: Count R1 once; completed refund USD 20; remaining paid amount USD 80.
2. O2: Completed refunds USD 0; request USD 30 shown separately.
3. R3: Unmatched exception; do not attach it to O1 or O2.

**Key check / 关键检查：** Matched remaining amount is USD 160; unmatched USD 10 remains unreconciled. This is an event exercise, not a revenue or accounting policy. 不据此确认收入。

## Change one input / 变式练习

Mark R2 completed: matched remaining amount becomes USD 130; R3 is still unresolved.
