# 用户反馈归类 / Triage product feedback

FORMWEFT practice card · 19/20 · 2026-09-14
...
Separate defects, requests and usability observations with evidence. 根据反馈判断需复现的问题与新需求。

Input fields / 输入字段：`feedback_id, exact_feedback, reproduction, expected_behavior, impact`

## Sample inputs / 样例输入

1. A: 'Import button does nothing' with repeatable steps on a sample file.
2. B: 'Could you add team folders?'
3. C: 'I could not find the export button' with no interaction details.
...
1. A: Defect candidate; preserve reproduction and ask for environment.
2. B: Feature request; no commitment or delivery date.
3. C: Discoverability observation; ask where the user looked.

**Key check / 关键检查：** Do not label every request as a bug or every complaint as verified. 输出类型、影响依据和下一步。

## Change one input / 变式练习

Add a note that A used an unsupported file type: reclassify only after checking the published input requirements.
